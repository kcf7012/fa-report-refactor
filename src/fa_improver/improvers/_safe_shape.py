...
#
#   title 帶(0.30~1.15 in)
#     └─ **真實客戶檔 0.97 in** ← 這才是這個常數真正的依據
#        MS / N160JCN 的「群組」(0.54~0.97,top 0.00~0.94)
#     └─ 合成 fixture synthetic_C 是 1.00(master/LeftTopDecoration,
#        0.00~1.00,top 0.00~0.50)。它是專案自己捏的、剛好一整吋的圓整數
#        (`build_synthetic_fixtures.py` 寫死 cx="914400"),**不該當主要依據**
#        —— 拿人為的圓整數當「跨檔最大值」會讓人合理懷疑常數是反推的。
#        所幸兩者導出同一個答案,見下方定值規則。
...
#        (`showMasterSp` 是另外查證的,量測腳本本身沒讀這個屬性)
#
# 定值規則:**量測最大值 + 0.20 in 緩衝,向上取到 0.05 的倍數**。
#
#   ⚠️ **「+0.20 緩衝」這個數字不是量測所得**,是自由參數 —— 與下面
#      `TITLE_MIN_WIDTH_INCH` 同級的判斷值,不享有母片量測的背書。
#      緩衝取 0.16~0.20 之間任一值,從 1.00 出發都會得到 1.20;取 0.25 就得 1.25。
#      **規則的自由度足以決定答案**,所以它必須被明白標示,不能混在量測結論裡。
#
#   title:0.97 + 0.20 = 1.17 → 1.20 ← 只用真實客戶檔就得到 1.20
#          1.00 + 0.20 = 1.20 → 1.20 ← 加計合成 fixture 也是 1.20
#          兩條路徑同解,**結論不依賴那個可疑的 1.00**;且與既有值相同,
#          是量測獨立落回同一個數,不是為了配合既有測試才這樣選。
#   body :1.12 + 0.20 = 1.32 → 1.35(既有的 `TITLE_SAFE_LEFT_INCH - 0.2` = 1.00
#          **不足**,比 260811 的 Picture 14 右緣還左 0.12 in,正文首字會壓在
#          母片裝飾上)
#
# ⚠️ 已知量測缺口(柔伊第八輪查證,轉入 P7):
#    - title 帶取的是 **fallback textbox** 的幾何(0.30~1.15),但
#      `_ensure_title_left_safe()` 只改 left/width、**保留原本的 top/height**,
#      被移動的原生 placeholder 實際可能佔到 0.30~1.57(synthetic_C 就是),
#      `1.15~1.57` 從沒被量測過 —— 而 260811 的 Picture 14 起點 y=1.23
#      正落在這段盲區裡(今天沒事只因它 right=1.12 < 1.20)。
#    - `_classify()` 的 `other` 桶(left ≤25% 但 right ≥50%)同樣不列入下限,
#      但不像 `full_bleed` 有「移了也閃不開」的論證,排除決策本身沒被辯護。
# ---------------------------------------------------------------------------
